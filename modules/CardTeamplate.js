...
function CardsTrain(ObjCards, N, K) {
  //N - номер массива в cards, K - номер объекта(карточки) в массиве N 
...
function CardsTrainWork() {
  const englishContent = document.querySelectorAll('.content-item-start');
  const rusiianContent = document.querySelectorAll('.content-item-end');
  const CONTENT = document.querySelector('.content-inner');
  const TimerAnimationRotation = 350;
  const TimerStartTrackingCursor = 400;
  let Data;
  function TimeOutB() {
    rusiianContent[Data].classList.add('passive');
  }
  function TimeOutA() {
    englishContent[Data].classList.remove('active');
  }
  function TimeOutMouseOut() {
    rusiianContent[Data].addEventListener('mouseleave', () => {
      rusiianContent[Data].classList.remove('passive');
  ...
  CONTENT.onclick = function (event) {
    ...
    if (target.tagName == 'IMG') {
      ...
      englishContent[Data].classList.add('active');
